...
def build_prompt(name, number, name_below=None):
    ...
    if number:
        ...
        parts.append(
            ...
            f'proportionally so all {len(number)} digits fit cleanly in the same area, all digits the same height '
            f'and style as each other. Do not add any logos, icons or marks inside or around the number.'
        )
    if name_below:
        parts.append(
            ...
            f'(the gap between the badge and this text must be tight, not a large empty space). '
