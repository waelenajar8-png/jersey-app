...
@app.route("/api/admin/influenceurs-summary")
@_require_admin_api
def api_admin_influenceurs_summary():
    """
    Version allégée de la liste des influenceurs pour le dashboard admin :
    juste ce qu'il faut pour afficher les cartes + ouvrir chaque espace.
    """
    try:
        data = r2_get_json(INFLUENCEURS_R2_KEY) or {}
        influenceurs = data.get("influenceurs", [])
        ...
        for inf in influenceurs:
            s = inf.get("stats") or {}
            sales = _safe_int(s.get("sales", 0))
            commission = _safe_float(s.get("commission", 0))
            total_sales += sales
            total_commission += commission
            tier_idx, next_tier, pct, _details = _compute_tier_progress(s)
            tier = INFLUENCER_TIERS[tier_idx]
            out.append({
                "id": inf.get("id"),
                "pseudo": inf.get("pseudo", "—"),
                ...
                "status": inf.get("status", 0),
                "promo_code": inf.get("promo_code", ""),
                "program_start_date": inf.get("program_start_date", ""),
                "last_synced_at": inf.get("last_synced_at", ""),
                "sales": sales,
                "sales_month": _safe_int(s.get("sales_month", 0)),
                "revenue": _safe_float(s.get("revenue", 0)),
                "commission": commission,
                "tier": {
                    "id": tier["id"], "name": tier["name"], "icon": tier["icon"], "color": tier["color"],
                    "next_name": next_tier["name"] if next_tier else None,
                    "next_sales": next_tier["requirements"]["sales"] if next_tier else None,
                    "progress_pct": pct,
                },
                "espace_url": f"{base_url}/espace/{_public_slug(inf)}",
            })

        # Tri par ventes cumulées décroissantes (les plus performants en premier)
        ...
        print(f"[ADMIN DASHBOARD] Erreur: {e}")
        return jsonify({"influenceurs": [], "totals": {"count": 0, "sales": 0, "commission": 0}, "error": str(e)})


@app.route("/api/admin/influenceur/<inf_id>/quick-update", methods=["POST"])
@_require_admin_api
def api_admin_quick_update(inf_id):
    """
    Mise à jour rapide d'un influenceur (code promo + date de départ programme)
    sans passer par le flux complet de sauvegarde de la liste entière —
    pensé pour une édition ponctuelle depuis le dashboard admin.
    """
    try:
        payload = request.json or {}
        promo_code = payload.get("promo_code")
        program_start_date = payload.get("program_start_date")
        ...
        with _influenceurs_lock:
            ...
            target = next((i for i in influenceurs if i.get("id") == inf_id), None)
            if not target:
                return jsonify({"success": False, "error": "Influenceur introuvable"}), 404

            if promo_code is not None:
                target["promo_code"] = promo_code.strip().upper()
            if program_start_date is not None:
                target["program_start_date"] = program_start_date
            target["lastModified"] = datetime.now(timezone.utc).isoformat()

            new_version = data.get("version", 0) + 1
            now_iso = datetime.now(timezone.utc).isoformat()
            ok = r2_put_json(INFLUENCEURS_R2_KEY, {
                "influenceurs": influenceurs, "version": new_version, "updated_at": now_iso,
            })
            if not ok:
                return jsonify({"success": False, "error": "écriture R2 échouée"}), 500
        ...
        print(f"[ADMIN DASHBOARD] Erreur quick-update: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
